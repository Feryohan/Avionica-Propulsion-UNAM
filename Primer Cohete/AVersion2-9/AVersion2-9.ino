//67% de almacenamiento
//64% de memoria dinámica
//Gyro y Magnetometro arreglado
//Quitamos GPS
//Se agrego divisor de voltaje, si la pila tiene menos de 8 V el buzzer sonará

//Despliegue con una altura por debajo de los 20 m de la alturaMaxima

//Muestra el estado del vuelo
  //A = Ascenso
  //D = Descenso
  //Q = Quieto
  //E = Explosión del paracaidas

//Conexiones Módulo MicroSD      Conexiones IMU: GY-80       Conexiones Relevador      Conexiones GPS       Conexiones Divisor de Voltaje
//   GND ---> GND                  3.3V ---> Vcc                  D8 ---> IN            3.3V ---> Vcc         2 resistencias de 10 k
//    5V ---> Vcc                   GND ---> GND                 GND ---> GND            GND ---> GND             Pin análogico A6
//   D12 ---> MISO                   A5 ---> SCL                  5V ---> Vcc          4(RX) ---> TXD
//   D11 ---> MOSI                   A4 ---> SDA                                       3(TX) ---> RXD
//   D13 ---> SCK
//   D10 ---> CS  
...
#define nAlturas 8                  //Numero de datos que se utilizaran para calcular la media movil
        //--------ADXL345---------
...
int divisorVoltaje = A6;       //Pin de entrada para el divisor de voltaje
int voltajePila;               //El valor que el analogRead en el pin A6 nos da
int numeroLectura = 1;         //Para obtener el numero de datos que queremos
word t0;                       //Tiempo al terminar de configurar
char Estado = ' ';             //Indica el estado del vuelo
int rango = 20;                //Indica cuantos metros por debajo de la altura maxima se activa el relevador
        //--------BMP085---------
    float temperature,pressure,altura;
    float alturaMaxima = 0; 
    float alturaAnterior = 0;  
//-->Alturas para el cálculo de la media móvil
    long arregloAlturas[nAlturas]; //arreglo para calcular la media movil 
...
float GaussXDigit = 0.00073;     // Gauss unit [G]
